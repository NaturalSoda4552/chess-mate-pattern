...
- 바로 앞 칸이 비어있을 경우, 앞으로 한 칸 전진할 수 있어야 한다.
- 초기 위치(2행)에 있고 앞 두 칸이 모두 비어있을 경우, 앞으로 두 칸 전진할 수 있어야 한다.
- 바로 앞에 다른 기물이 있으면 전진할 수 없다.
- 대각선 앞에 적군 기물이 있을 경우, 해당 칸으로 이동(공격)할 수 있어야 한다.
- 대각선 앞에 아군 기물이 있거나 비어있을 경우, 해당 칸으로 이동할 수 없다.

##### Rook.js (룩)

- 빈 보드에서 상하좌우 직선으로 움직일 수 있는 모든 칸을 반환해야 한다.
- 경로 중간에 아군 기물이 있으면, 그 너머로는 이동할 수 없다.
- 경로 중간에 적군 기물이 있으면, 그 칸까지는 이동할 수 있지만, 그 너머로는 이동할 수 없다.

##### Knight.js (나이트)

- L자 형태로 움직일 수 있는 최대 8개의 칸을 반환해야 한다.
- 경로 중간에 다른 기물이 있어도 뛰어넘을 수 있다.
- 도착 지점에 아군 기물이 있으면 그 칸으로 이동할 수 없다.
- 도착 지점에 적군 기물이 있으면 그 칸으로 이동할 수 있다.

##### Bishop.js (비숍)

- 빈 보드에서 대각선으로 움직일 수 있는 모든 칸을 반환해야 한다.
- 경로 중간에 아군 기물이 있으면, 그 너머로는 이동할 수 없다.
- 경로 중간에 적군 기물이 있으면, 그 칸까지는 이동할 수 있지만, 그 너머로는 이동할 수 없다.

##### Queen.js (퀸)

- 상하좌우 및 대각선(룩과 비숍의 움직임)으로 움직일 수 있는 모든 칸을 반환해야
  한다.
- 경로 중간에 적군 기물이 있으면, 그 칸까지는 이동할 수 있지만, 그 너머로는 이동할 수 없다.

##### King.js (킹)

- 주변 8개의 모든 칸으로 이동할 수 있어야 한다.
- 도착 지점에 아군 기물이 있으면 이동할 수 없다.
