...
- `MVC 패턴` 적용 및 `React`를 `View`로 활용하여 UI 체스 게임을 렌더링한다.
...
- 왼쪽 패턴 리스트에서 도전하고 싶은 체스 체크메이트 패턴을 클릭한다.
  - 각 패턴은 1수 또는 2수를 요구한다.

### 기물 선택 및 이동

- 오른쪽의 설명을 보고, 기물을 이동시켜 퀴즈를 푼다.
  - 플레이어의 색은 항상 백이며, 기물 클릭 시 이동할 수 있는 칸이 초록색으로 표시된다.
  - 플레이어의 수가 정답일 경우, 흑이 턴을 진행한 후 다시 플레이어의 턴으로 돌아온다.
  - 플레이어의 수가 오답일 경우, 다시 이전 수로 돌아간다.
  - 플레이어의 수가 체크메이트일 경우, 그 퀴즈는 완료된다.
...
- 플레이어의 턴은 `백`으로 고정이므로, 백 기물만 움직일 수 있다.
- 기물 클릭 시 `해당 기물의 유효한 이동 칸`이 하이라이트된다.
- 사용자는 기물을 이동이 유효한 칸으로만 이동시킬 수 있다.
- 1수 내로 체크메이트하는 경우, 퀴즈는 완료된다.
- 2수 이상으로 체크메이트하는 경우, 턴을 교체하여 흑 차례를 진행하고 이후 다시 사용자의 유효한 수를 받는다.
...
- 정답일 경우:
  - 다음 수가 존재할 시 상대방 수를 진행하며, 다시 플레이어 턴을 진행할 수 있다.
- 오답일 경우:
  - 체스판은 자동으로 `체크포인트 상태`로 되돌아간다.
- 체크메이트일 경우:
  - 퀴즈가 완료된다.
- 각 판별 결과에 따라 설명 패널의 색을 다르게 준다. (순서대로 초록, 빨강, 노랑)
- 각 판별 결과에 따라 설명 패널의 텍스트의 애니메이션을 다르게 준다.
